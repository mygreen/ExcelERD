...
' 2015/09/02 ver 1.1   Excel2007/2010に対応
...
Public Const TIPS_DDL_COMMENT               As String = "DDLに出力する行単位のコメント文字列を指定してください。""--"" など。"
...
Public Function getMessage(ByVal msgId As Long) As String
    ...
    Select Case msgId
        ...
        Case ERR_NOT_WRITE_FOLDER
            result = "フォルダ [{1}] は書き込み可能ではありません。"
        Case ERR_NOT_WRITE_FILE
            result = "ファイル [{1}] は書き込み可能ではありません。"
        Case ERR_NOT_EXIST_FILE
            result = "ファイル [{1}] は存在しません。"
    End Select
